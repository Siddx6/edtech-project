...
export async function analyzeExam(req, res) {
  try {
    const questionPaperFile = req.files?.question_paper?.[0];
    const answerSheetFile = req.files?.answer_sheet?.[0];
    ...
    const calculatedTotalScored = extractedData.questions.reduce(
      (sum, question) => sum + (question.marks_scored || 0),
      0,
    );
...
function normalizeFallbackStudentInfo(body) {
...
function extractTopicLabels(topics) {
...
function countWeakTopics(chapters) {
...
function mergeStudentInfo(extractedStudentInfo, suggestedStudentInfo) {
  const merged = { ...normalizeStudentInfo(extractedStudentInfo) };
...
function normalizeExtractedQuestions(questions) {
...
function normalizeBreakdown(breakdown, marksScored) {
...
function normalizeQuestionStatus(status, marksScored) {
