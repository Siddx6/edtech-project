 ...
 * @param {string} student_id - MongoDB student ID
 * @param {string} subject - Subject name
 * @param {number} questionCount - Number of questions (optional, defaults to 10)
 * @param {string} difficulty - Difficulty level (optional, defaults to 'adaptive')
 */
export async function generatePracticeQuiz({
  ...
  if (!practiceContext.weak_chapters || !practiceContext.weak_chapters.length) {
    throw new Error(
      "No weak topics are currently recorded for this student in the selected subject",
    );
  }
  ...
  return _generatePracticeQuizFromContext({
    ...
    weakChapters: practiceContext.weak_chapters,
...
function buildPracticeQuizPrompt({
  ...
  return `
    ...
    - Language should match the subject naturally
...
function validatePracticeQuiz(quizJSON, expectedQuestionCount) {
  const errors = [];
...
function normalizePracticeQuiz({
  ...
  return {
    ...
    questions: quizJSON.questions.map((question, index) => ({
      question_no: index + 1,
